...
impl<O: Write> Write for KcpOutput<O> {
    fn write(&mut self, data: &[u8]) -> io::Result<usize> {
        tracing::trace!("[RO] {} bytes", data.len());
        self.0.write(data)
    }
...
impl<Output> Kcp<Output> {
    ...
    pub fn recv(&mut self, buf: &mut [u8]) -> KcpResult<usize> {
        ...
        if peeksize > buf.len() {
            tracing::debug!("recv peeksize={peeksize} bufsize={} too small", buf.len());
    ...
    pub fn input(&mut self, buf: &[u8]) -> KcpResult<usize> {
        ...
        tracing::trace!("[RI] {} bytes", buf.len());
        ...
        while buf.remaining() >= KCP_OVERHEAD {
            ...
            if conv != self.conv {
                ...
                if self.input_conv {
                    tracing::debug!("input conv={conv} updated, original conv={}", self.conv);
                    self.conv = conv;
                    self.input_conv = false;
                } else {
                    tracing::debug!("input conv={conv} expected conv={} not match", self.conv);
            ...
            match cmd {
                ...
                _ => {
                    tracing::debug!("input cmd={cmd} unrecognized");
            ...
            match cmd {
                ...
                KCP_CMD_PUSH => {
                    tracing::trace!("input psh: sn={sn} ts={ts}");
                ...
                KCP_CMD_WASK => {
                    // ready to send back IKCP_CMD_WINS in ikcp_flush
                    // tell remote my window size
                    tracing::trace!("input probe");
                    self.probe |= KCP_ASK_TELL;
                }
                KCP_CMD_WINS => {
                    // Do nothing
                    tracing::trace!("input wins: {wnd}");
                }
                _ => unreachable!(),
            }
    ...
    pub fn set_mtu(&mut self, mtu: usize) -> KcpResult<()> {
        if mtu < 50 || mtu < KCP_OVERHEAD {
            tracing::debug!("set_mtu mtu={mtu} invalid");
...
impl<Output: Write> Kcp<Output> {
    ...
    pub fn flush_ack(&mut self) -> KcpResult<()> {
        if !self.updated {
            tracing::debug!("flush updated() must be called at least once");
