...
use nod_krai_gi_entity::common::{
    AbilityModifierController, FightProperties, InstancedAbilities, InstancedModifiers,
};
use nod_krai_gi_event::ability::{AbilityActionApplyModifierEvent, ExecuteActionEvent};
use std::collections::HashMap;

use crate::util::eval_option;

pub fn ability_action_apply_modifier_event(
    mut events: MessageReader<AbilityActionApplyModifierEvent>,
    mut abilities_query: Query<(
    ...
    for event in events.read() {
        ...
        let Ok((mut abilities, mut modifiers, fight_props)) =
            abilities_query.get_mut(ability_entity)
        else {
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_apply_modifier_event] Entity {} not found or has no InstancedAbilities",
                    ability_entity
                );
            }
            continue;
        ...
        let ability = match abilities.list.get_mut(ability_index as usize) {
            ...
            None => {
                if GAME_SERVER_CONFIG.plugin.ability_log {
                    tracing::debug!(
                        "[ability_action_apply_modifier_event] Ability index {} not found for entity {}",
                        ability_index,
                        ability_entity
                    );
                }
        ...
        let Some(ability_data) = &ability.ability_data else {
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_apply_modifier_event] No ability_data for ability index {} on entity {}",
                    ability_index,
                    ability_entity
                );
            }
        ...
        let Some((_, modifier_def)) = ability_data
            .modifiers
            .iter()
            .find(|(name, _)| name.as_str() == modifier_name)
        else {
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_apply_modifier_event] Modifier {} not found in ability {}",
                    modifier_name,
        ...
        let think_interval = modifier_def
            .think_interval
            .as_ref()
            .map(|t| eval_option(&ability, None, &Some(t.clone()), 0.0));

        let modifier_name_intern = InternString::from(modifier_name);

        if ability.modifiers.contains_key(&modifier_name_intern) {
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_apply_modifier_event] Modifier {} already exists on entity {}, skipping",
                    modifier_name,
                    target_entity
                );
            }
        ...
        let instanced_modifier_id =
            ability.instanced_ability_id.unwrap_or(0) * 1000 + ability.modifiers.len() as u32 + 1;
        ...
        ability
            .modifiers
            .insert(modifier_name_intern, instanced_modifier_id);
        ...
        if GAME_SERVER_CONFIG.plugin.ability_log {
            tracing::debug!(
                "[ability_action_apply_modifier_event] Applied modifier: {} to entity {} (id: {}, duration: {}, think_interval: {:?})",
                modifier_name,
                target_entity,
                instanced_modifier_id,
                duration,
