...
use nod_krai_gi_event::ability::AbilityActionRemoveUniqueModifierEvent;
use common::string_util::InternString;

pub fn ability_action_remove_unique_modifier_event(
    mut events: MessageReader<AbilityActionRemoveUniqueModifierEvent>,
    mut abilities_query: Query<(&mut InstancedAbilities, &mut InstancedModifiers, Option<&mut FightProperties>)>,
) {
    for AbilityActionRemoveUniqueModifierEvent(
        ability_index,
        ability_entity,
        action,
        _ability_data,
    ...
    ) in events.read()
    {
        ...
        let Ok((mut abilities, mut modifiers, fight_props)) = abilities_query.get_mut(*ability_entity) else {
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_remove_unique_modifier_event] Entity {} not found or has no InstancedAbilities",
                    ability_entity
                );
            }
            continue;
        ...
        let Some(ability) = abilities.list.get_mut(*ability_index as usize) else {
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_remove_unique_modifier_event] Ability index {} not found for entity {}",
                    ability_index,
                    ability_entity
                );
            }
        ...
        if let Some(&modifier_id) = ability.modifiers.get(&modifier_name_intern) {
            if let Some(modifier) = modifiers.get(&modifier_id) {
                if !modifier.property_deltas.is_empty() {
                    if let Some(mut fight_props) = fight_props {
                        for (prop_type, delta) in &modifier.property_deltas {
                            ...
                            if GAME_SERVER_CONFIG.plugin.ability_log {
                                tracing::debug!(
                                    "[ability_action_remove_unique_modifier_event] Reverted property {:?} by {} for entity {:?}",
                                    prop_type,
                                    delta,
                                    target_entity
                                );
                ...
                if GAME_SERVER_CONFIG.plugin.ability_log {
                    tracing::debug!(
                        "[ability_action_remove_unique_modifier_event] Removed unique modifier: {} (id: {}) from entity {}",
                        modifier_name,
                        modifier_id,
                        target_entity
                    );
            ...
            if GAME_SERVER_CONFIG.plugin.ability_log {
                tracing::debug!(
                    "[ability_action_remove_unique_modifier_event] Unique modifier {} not found on entity {}",
                    modifier_name,
                    target_entity
                );
            }
