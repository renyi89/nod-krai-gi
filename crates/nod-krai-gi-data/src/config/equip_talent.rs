...
static EQUIP_TALENT_CONFIG_MAP: OnceLock<HashMap<String, TalentConfig>> = OnceLock::new();
...
pub fn get_equip_talent_config(name: &str) -> Option<&TalentConfig> {
    EQUIP_TALENT_CONFIG_MAP.get().unwrap().get(name)
}

pub fn iter_equip_talent_config_map() -> std::collections::hash_map::Iter<'static, String, TalentConfig> {
